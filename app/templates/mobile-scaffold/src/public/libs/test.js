...
(function (w, d) { if (!window.encrypt) { window.encrypt = {} } encrypt = { getEncryptStr: function (data, sk) { var unCryptStr = ""; if (typeof data == 'object') { for (var key in data) { unCryptStr = unCryptStr + '' + key + '=' + data[key] + '&' } 
var len = unCryptStr.length - 1;
console.log(CryptoJS,"CryptoJS"); 
var hmacSha1=CryptoJS.enc.Utf8.parse(CryptoJS.HmacSHA1(unCryptStr.substr(0, len), sk))
return CryptoJS.enc.Base64.stringify(hmacSha1) } else { return '请检查传入json对象是否正确' } } } })(window, document);