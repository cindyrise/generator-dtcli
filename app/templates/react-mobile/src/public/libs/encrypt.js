/*
encrypt.getEncryptStr(message,sk);
message:待加密字符串;
密钥：sk;
return 加密字符串;
*/
...
(function (w, d) {
    if (!window.encrypt) { window.encrypt = {} } encrypt = {
        getEncryptStr: function (data, sk) {
            console.log(typeof data);
            if (typeof data == 'object') {
                console.log(1231)
                var unCryptStr = "", sortFiled = [], encryObj = {};
                for (var key in data) {
                    sortFiled.push(key);
                };
                sortFiled.sort();
                for (var i = 0; i < sortFiled.length; i++) {
                    encryObj[sortFiled[i]] = data[sortFiled[i]];
                }
                for (var keyObj in encryObj) {
                    unCryptStr = unCryptStr + '' + keyObj + '=' + data[keyObj] + '&';
                }
                var len = unCryptStr.length - 1;
                var hmacSha1Str=CryptoJS.enc.Utf8.parse(CryptoJS.HmacSHA1(unCryptStr.substr(0, len), sk))
                console.log(hmacSha1Str,unCryptStr.substr(0, len));
                return CryptoJS.enc.Base64.stringify(hmacSha1Str)
            } else { return '请检查传入json对象是否正确' }
        }
    }
})(window, document);